...
p("For each number you input u will get number of bulls and cows. Let's say the actual number is 3719 and the number you input is 1629. For this number computer will responds with ",style = "font-family: 'times'; font-si16pt",strong("1 B 2 C"), em("(1 Bull and 2 Cows) 1 Bull implies 1 digit among the 4 you guess is correct and in correct position and 2 cows implies 2 digits are correct but are not in the correct position")),
  ...
  observeEvent(input$submit,{
    ...
    #instruct message in case NA
    inputlist$instruct=InstructMessage(guessnumC)
    # Checking the cows and bulls of the guess
  ...
  #Output
  output$instructmessage<-renderUI({
    if(!is.na(inputlist$instruct)){tagList(
      p(strong("The input number dosent comply with the instructions given:")),
      p(" 1) The number must be 4 digit or 3 digit (if you want 0 in the first place say 0123 can be given as 123)."),
      p(" 2) There should not be any alphabets or symbols in the number input."),
      p(" 3) The digits in the input number should be unique (e.g. 1214 not allowed as 1 is repeating).")
    )}
  })
